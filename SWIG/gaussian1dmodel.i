...
class Gaussian1dModel : public TermStructureConsistentModel {
    public:
        const QuantLib::ext::shared_ptr<StochasticProcess1D> stateProcess() const;
        ...
        const Real forwardRate(const Date &fixing,
                               const Date &referenceDate = Null<Date>(),
                               const Real y = 0.0,
                               QuantLib::ext::shared_ptr<IborIndex> iborIdx =
                                   QuantLib::ext::shared_ptr<IborIndex>()) const;

        const Real swapRate(const Date &fixing, const Period &tenor,
                            const Date &referenceDate = Null<Date>(),
                            const Real y = 0.0,
                            QuantLib::ext::shared_ptr<SwapIndex> swapIdx =
                                QuantLib::ext::shared_ptr<SwapIndex>()) const;
...
class Gsr : public Gaussian1dModel {
  ...
  public:
    ...
    Array params() const;
    void calibrate(
            const std::vector<QuantLib::ext::shared_ptr<CalibrationHelper> >& instruments,
            OptimizationMethod& method, const EndCriteria& endCriteria,
            const Constraint& constraint = Constraint(),
            const std::vector<Real>& weights = std::vector<Real>(),
...
class MarkovFunctional : public Gaussian1dModel {
  ...
  public:
    ...
    void calibrate(
        const std::vector<QuantLib::ext::shared_ptr<CalibrationHelper> > &helper,
        OptimizationMethod &method, const EndCriteria &endCriteria,
        const Constraint &constraint = Constraint(),
        const std::vector<Real> &weights = std::vector<Real>(),
        const std::vector<bool> &fixParameters = std::vector<bool>());  
...
class Gaussian1dSwaptionEngine : public PricingEngine {
  ...
  public:
    ...
    Gaussian1dSwaptionEngine(const QuantLib::ext::shared_ptr<Gaussian1dModel> &model,
...
class Gaussian1dJamshidianSwaptionEngine : public PricingEngine {
  public:
    Gaussian1dJamshidianSwaptionEngine(const QuantLib::ext::shared_ptr<Gaussian1dModel>& model);
};
...
class Gaussian1dNonstandardSwaptionEngine : public PricingEngine {
  ...
  public:
    ...
    Gaussian1dNonstandardSwaptionEngine(
            const QuantLib::ext::shared_ptr<Gaussian1dModel> &model,
            const int integrationPoints = 64, const Real stddevs = 7.0,
            const bool extrapolatePayoff = true,
            const bool flatPayoffExtrapolation = false,
