...
class GJRGARCHEngineTest(unittest.TestCase):
    def setUp(self):
        ...
        self.analytic = [[[15.4315, 10.5552, 5.9625, 2.3282, 0.5408, 0.0835],
        ...
        self.mc_values = [[[15.4332, 10.5453, 5.9351, 2.3521, 0.5597, 0.0776],
    ...
    def testOptionPricing(self):
        ...
        for k in range(3):
            ...
            m1 = self.beta + (self.alpha + self.gamma * ql.CumulativeNormalDistribution()(lambda_value)) * (
            ...
            garch = ql.GJRGARCHProcess(self.risk_free_handle, self.dividend_yield_handle, quote, v0, self.omega,
                                       self.alpha, self.beta, self.gamma, lambda_value, self.daysPerYear)
            garch_model = ql.GJRGARCHModel(garch)
            analytic_engine = ql.AnalyticGJRGARCHEngine(garch_model)
            mc_engine = ql.MCEuropeanGJRGARCHEngine(process=garch, traits='pseudorandom', timeStepsPerYear=20,
                                                    requiredTolerance=0.02, seed=1234)
            for i in range(2):
                for j in range(6):
...
class GJRGARCHCalibrationTest(unittest.TestCase):
    def setUp(self):
        ...
        self.gamma = 0.059
        self.daysPerYear = 365.0
        self.Volatility = [0.6625, 0.4875, 0.4204, 0.3667, 0.3431, 0.3267, 0.3121, 0.3121,
    ...
    def testCalibration(self):
        ...
        m1 = self.beta + (self.alpha + self.gamma * ql.CumulativeNormalDistribution()(self.lambda_value)) * (
        ...
        for s in range(3, 10):
            for m in range(0, 3):
                ...
                heston_helper = ql.HestonModelHelper(maturity, self.calendar, self.s0, self.strike[s], vol,
                                                     self.risk_free_ts,  self.dividend_yield_handle,
                                                     ql.BlackCalibrationHelper.ImpliedVolError)
                helpers.append(heston_helper)

        new_garch_process = ql.GJRGARCHProcess(self.risk_free_ts, self.dividend_yield_handle,
                                               ql.QuoteHandle(ql.SimpleQuote(self.s0)),
                                               v0, self.omega, self.alpha, self.beta, self.gamma, self.lambda_value,
                                               self.daysPerYear)
