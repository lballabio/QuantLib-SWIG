...
isdaRateHelpers = [ql.DepositRateHelper(dep_quotes[i],dep_tenors[i]*ql.Period(ql.Monthly),2,ql.WeekendsOnly(),
                                        ql.ModifiedFollowing,False,ql.Actual360()) for i in range(len(dep_tenors))]
...
isda_ibor = ql.IborIndex('IsdaIbor',3*ql.Period(ql.Monthly),2,ql.USDCurrency(),ql.WeekendsOnly(),
                         ql.ModifiedFollowing,False,ql.Actual360())
isdaRateHelpers = isdaRateHelpers + [ql.SwapRateHelper(swap_quotes[i],swap_tenors[i]*ql.Period(ql.Annual),
...
for i in range(0,len(termDates)):
    for j in range(0,2):
        for k in range(0,2):
            ...
            cdsSchedule = ql.Schedule(tradeDate+1,termDates[i],3*ql.Period(ql.Monthly),ql.WeekendsOnly(),
            ...
            h = quotedTrade.impliedHazardRate(0,discountCurve,ql.Actual365Fixed(),recoveries[k],1e-10,ql.CreditDefaultSwap.ISDA)
            
            probabilityCurve.linkTo(ql.FlatHazardRate(0,ql.WeekendsOnly(),ql.QuoteHandle(ql.SimpleQuote(h)),ql.Actual365Fixed()))
            
            #engine = ql.MidPointCdsEngine(probabilityCurve,recoveries[k],discountCurve)
            ...
            print("Hazard:",h,"Upfront:",upfront,"Distance:",abs(upfront-markitValues[l]),"Tolerance:",tolerance,abs(upfront-markitValues[l])<tolerance)
